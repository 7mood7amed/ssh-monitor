...
# Allowed sources: auth.log*, apache2/access.log*, apache2/error.log*
# =========================================
...
def allowed_logs_where_sql(alias: str = "l") -> str:
    ...
    col = f"{alias}.source"
    return (
        f"({col} LIKE %s OR {col} LIKE %s OR {col} LIKE %s)"
    )


def allowed_logs_where_params() -> Tuple[str, str, str]:
    return (
...
def get_web_logs():
    ...
    try:
        ...
        if status and status.isdigit():
            # quick regex to match status in the line: ... "GET /x HTTP/1.1" 200 ...
            where.append("l.message ~ %s")
            params.append(r'"\s+\d{3}\s+.*' if False else rf'"\s+{status}\s+')
...
def export_logs():
    ...
    Optional: service=ssh|web|apache_error
    """
    try:
