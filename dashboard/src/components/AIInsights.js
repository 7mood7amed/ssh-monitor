...
function getPatternCount(item) {
  return safeArray(item?.probable_patterns).length + safeArray(item?.patterns).length;
}
...
function buildEntityDetails(item) {
  ...
  return [
    ...
    { label: "Sources", value: safeArray(item?.sources).join(", ") || "—" },
    {
      label: "Patterns",
      value:
        safeArray(item?.probable_patterns).join(", ") ||
        safeArray(item?.patterns).join(", ") ||
        "—",
    },
  ...
  return (
    <div className={`ai-finding-card ai-finding-${tone}`}>
      <div className="ai-finding-header">
        ...
        {item && <SectionChip label="score" value={getItemScore(item)} tone={tone} />}
      </div>
          ...
          {items.slice(0, 5).map((item, index) => (
            <div className="ai-mini-row" key={getItemKey(item, index)}>
              ...
              <div className="ai-mini-side">
                <SectionChip label="score" value={getItemScore(item)} tone={getScopeTone(item)} />
              </div>
            </div>
          ))}
        </div>
...
function buildReasoningPoints(data, correlationData) {
  ...
  if (riskLevel) {
    points.push(
      `Overall risk is ${String(riskLevel).toUpperCase()} based on recent alerts, authentication failures, suspicious web activity, and correlated evidence.`
    );
  }
  ...
  if (topActivity?.ip) {
    points.push(
      `Top activity is linked to ${topActivity.ip}, classified as ${topActivity.assessment || "unknown activity"}.`
    );
  }
  ...
  if (safeArray(topActivity?.probable_patterns).length > 0) {
    points.push(`Detected pattern(s): ${safeArray(topActivity.probable_patterns).join(", ")}.`);
  }

  if ((totals?.ssh_failures ?? 0) > 0) {
    points.push(
      `${totals.ssh_failures} SSH failed login event(s) were observed in the selected window.`
    );
  }

  if ((totals?.ftp_failures ?? 0) > 0) {
    points.push(`${totals.ftp_failures} FTP failed login event(s) were observed in the selected window.`);
  }

  if ((totals?.alerts ?? 0) > 0) {
    points.push(`${totals.alerts} alert record(s) were generated by the rule-based detection layer.`);
  }
  ...
  if (topInternal?.ip) {
    points.push(`Internal/trusted host activity currently maps to ${topInternal.ip}.`);
  }

  if (!topExternal?.ip && !topInternal?.ip) {
    points.push("No dominant correlated source was identified in the current analysis window.");
  }

  if (correlatedIps.length > 1) {
    points.push(`${correlatedIps.length} correlated IP/entities were included in the cross-source analysis set.`);
  }
...
function buildVerdict(data) {
  const risk = String(data?.risk?.level || "unknown").toLowerCase();
  const totals = data?.report_data?.totals || {};
  const top = data?.report_data?.top_activity;

  if (risk === "critical") {
    return "Critical risk activity detected. Immediate investigation is required.";
  }
  ...
  if ((totals?.ftp_failures ?? 0) > 5) {
    return "Likely brute-force activity detected with repeated FTP failures.";
  }
...
function getCampaignAssessment(data, correlationData) {
  ...
  const multiSourceCount = safeArray(
    ...
    data?.correlations?.multi_source_ips ||
  ...
  if (multiSourceCount >= 2 || sshFailures >= 6 || ftpFailures >= 6) {
    return {
      ...
      summary: "Several evidence sources indicate suspicious activity that should be investigated.",
    };
  }
  ...
  if (alerts > 0) {
    return {
      ...
      summary: "Detection rules were triggered during the selected window. Analyst review is recommended.",
    };
  }

  return {
    severity: "low",
    title: "No Active Campaign Indicators",
    summary: "No strong coordinated threat indicators were detected in the selected time window.",
  };
}

function getCampaignTone(severity) {
...
function getDetectedPatterns(analyzeData, correlationData) {
  const topActivity = analyzeData?.report_data?.top_activity;
  const topCorrelations = getCorrelations(correlationData);

  const patterns = [
    ...safeArray(topActivity?.probable_patterns),
    ...safeArray(topActivity?.patterns),
    ...topCorrelations.flatMap((item) => [
      ...safeArray(item?.probable_patterns),
      ...safeArray(item?.patterns),
    ]),
  ];
...
function getRecommendedAction(item) {
  if (!item) {
    return "Select an entity to view recommended analyst action.";
  }

  const sshFailures = item?.ssh_failures ?? 0;
  const ftpFailures = item?.ftp_failures ?? 0;
  const webHits = item?.web_hits ?? 0;
  const alertCount = item?.alert_count ?? 0;
  const nmapPorts = item?.nmap_port_count ?? 0;
  const sourceCount = getSourceCount(item);

  if (sshFailures >= 5) {
    return "Prioritize SSH investigation. Review failed login attempts, usernames used, and source IP behavior.";
  }

  if (ftpFailures >= 5) {
    return "Prioritize FTP investigation. Review repeated failed FTP logins and check whether the same user or IP appears multiple times.";
  }

  if (alertCount > 0 && sourceCount >= 2) {
    return "Treat this as correlated suspicious activity. Review linked alerts and compare activity across multiple evidence sources.";
  }

  if (nmapPorts > 0) {
    return "Review Nmap findings. Confirm whether the exposed services are expected and remove unnecessary open ports.";
  }

  if (webHits > 20) {
    return "Review web activity. Check for suspicious paths, repeated 404 responses, admin-page probing, or enumeration behavior.";
  }

  return "Review related evidence and monitor this entity for repeated activity across SSH, FTP, Web, Nmap, or alerts.";
}

function getCampaignSources(analyzeData, correlationData) {
  const topActivity = analyzeData?.report_data?.top_activity;
  const topCorrelations = getCorrelations(correlationData);

  const sources = [
    ...safeArray(topActivity?.sources),
    ...topCorrelations.flatMap((item) => safeArray(item?.sources)),
  ];
...
export default function AIInsights({ refreshTrigger }) {
  ...
  const campaign = getCampaignAssessment(analyzeData, correlationData);
  const detectedPatterns = getDetectedPatterns(analyzeData, correlationData);
  const campaignSources = getCampaignSources(analyzeData, correlationData);
  const selectedEntityDetails = buildEntityDetails(selectedEntity);
  const recommendedAction = getRecommendedAction(selectedEntity);
  ...
  return (
    <div className="content ai-insights-wrap">
      <div className="card ai-card">
        ...
        >
          <span className="badge">Risk: {getRiskLevel(analyzeData)}</span>
          <span className="badge">Score: {getRiskScore(analyzeData)}</span>
        ...
        <div className="ai-findings-grid">
          ...
          <div
            onClick={() => topExternal && setSelectedEntity(topExternal)}
          ...
          <div className="ai-finding-card ai-finding-neutral">
            <div className="ai-finding-header">
              <div className="ai-finding-title">Multi-Service IPs</div>
        ...
        <div className="ai-verdict">AI Verdict: {verdict}</div>

        <div className={`campaign-banner campaign-${campaign.severity}`}>
          <div className="campaign-left">
            <div className="campaign-label">Campaign Assessment</div>
            ...
            {campaignSources.length > 0 && (
              <div className="campaign-sources">
                ...
                {campaignSources.map((source, index) => (
                  <span className="campaign-source-pill" key={`${source}-${index}`}>
                    {source}
                  </span>
                ))}
              </div>
          ...
          <div className="campaign-right">
            <SectionChip
              label="priority"
              value={campaign.severity.toUpperCase()}
              tone={getCampaignTone(campaign.severity)}
            />

            <SectionChip label="entities" value={totalCorrelations} />
          </div>
        </div>

        <div className="ai-patterns-card">
          <div className="ai-patterns-header">
            <div>
              <div className="ai-patterns-title">Attack Patterns Detected</div>
              <div className="ai-patterns-subtitle">
                Pattern indicators extracted from correlated evidence
              </div>
            </div>

            <SectionChip label="patterns" value={detectedPatterns.length} />
          </div>

          {detectedPatterns.length === 0 ? (
            <div className="empty">No attack patterns detected in this window.</div>
          ) : (
            <div className="ai-patterns-list">
              {detectedPatterns.map((pattern, index) => {
                ...
                const patternTone =
                  lowerPattern.includes("brute") || lowerPattern.includes("failed")
                    ? "pattern-danger"
                    : lowerPattern.includes("exposed") ||
                      lowerPattern.includes("service") ||
                      lowerPattern.includes("port")
                      ? "pattern-warning"
                      : "pattern-info";

                return (
                  <span
                    className={`ai-pattern-pill ${patternTone}`}
                    key={`${pattern}-${index}`}
                  >
                    ✓ {pattern}
                  </span>
        ...
        <div className="ai-investigation-card">
          <div className="ai-investigation-header">
            <div>
              <div className="ai-investigation-title">Incident Drilldown</div>
              <div className="ai-investigation-subtitle">
                Click any correlated entity to review its evidence profile and suggested analyst action.
              </div>
            </div>
            {selectedEntity && (
              <button type="button" className="btn" onClick={() => setSelectedEntity(null)}>
                Clear Selection
              </button>
            )}
          </div>
          ...
          {!selectedEntity ? (
            ...
            <>
              <div className="ai-investigation-highlight">
                Investigating: <span className="mono">{getItemIp(selectedEntity)}</span>
              </div>

              <div className="ai-action-box">
                <div className="ai-action-title">Recommended Analyst Action</div>
                <div className="ai-action-text">{recommendedAction}</div>
              </div>
              ...
              {topCorrelations.length === 0 ? (
                <tr>
                  <td colSpan={5} className="empty">
                    {loading ? "Loading top correlations..." : "No correlation data returned."}
                  </td>
                </tr>
              ) : (
